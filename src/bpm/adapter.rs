...
pub fn analyze_bpm_for_file(path: &str) -> Result<Option<f32>> {
    // Decode first to get raw samples
    let (samples, sample_rate, channels) = decode_to_vec(path)?;
    
    // Use the decoded data for detection
...
/// Decodes file to memory, IGNORING header metadata if it conflicts with real audio.
pub fn decode_to_vec(path: &str) -> Result<(Vec<f32>, u32, usize)> {
    ...
    // Initial guess from header (might be wrong)
    ...
    // Track if we have validated the format yet
    ...
    loop {
        ...
        // --- CRITICAL FIX: Update Sample Rate/Channels from Real Data ---
        if !format_verified {
